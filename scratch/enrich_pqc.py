...
# Enrichment data for all 13 PQC lessons
enrichments = {
    "quantum-foundations": {
        "pitfalls": [
    ...
    "shor-grover-threats": {
        ...
        "references": [
            ...
            {
                "title": "NIST IR 8309: Status Report on Post-Quantum Cryptography",
                "source": "NIST CSRC",
                "url": "https://doi.org/10.6028/NIST.IR.8309",
                "description": "Comprehensive federal review evaluating the quantum vulnerability of classical cryptographic algorithms.",
                "type": "standard"
            }
        ]
    },
    "lattice-foundations": {
        ...
        "workedExample": {
            ...
            "steps": [
                ...
                {"label": "4. Compute b₂*", " detail\: "b₂* = b₂ − μ₂₁ b₁* = (2, 1) − (0.5, 1.5) = (1.5, -0.5). Verified orthogonal: (1, 3) · (1.5, -0.5) = 1.5 − 1.5 = 0."}
            ],
        ...
        "references": [
            ...
            {
                "title": "Lenstra, Lenstra & Lovász: Factoring Polynomials with Rational Coefficients (LLL Algorithm, 1982)",
                "source": "Mathematische Annalen",
                "url": "https://doi.org/10.1007/BF01457454",
                "description": "The seminal paper establishing polynomial-time lattice basis reduction.",
                "type": "paper"
            }
        ]
    },
    "lwe-module-lwe": {
        ...
        "references": [
            ...
            {
                "title": "Lyubashevsky, Peikert & Regev: On Ideal Lattices and Ring-LWE (Eurocrypt 2010)",
                "source": "IACR Eurocrypt",
                "url": "https://eprint.iacr.org/2012/230.pdf",
                "description": "Introduced the Ring Learning With Errors problem, reducing key sizes by orders of magnitude.",
                "type": "paper"
            }
        ]
    },
    "fips203-ml-kem": {
        ...
        "references": [
            ...
            },
            {
                ...
                "description": "Complete mathematical specification, test vectors, and C reference implementation for Kyber.",
                "type": "spec"
            }
        ]
    },
    "fips204-ml-dsa": {
        ...
        "references": [
            ...
            {
                "title": "Vadim Lyubashevsky: Fiat-Shamir with Aborts: Applications to Lattice Signatures (Asiacrypt 2009)",
                "source": "IACR Asiacrypt",
                "url": "https://eprint.iacr.org/2009/398.pdf",
                "description": "The foundational mathematical breakthrough eliminating trapdoor sampling in lattice signatures.",
                "type": "paper"
            }
        ]
    },
    "fips206-fn-dsa": {
        ...
        "references": [
            ...
            {
                "title": "Fouque et al.: Falcon: Fast-Fourier Lattice-based Compact Signatures over NTRU",
                "source": "Falcon Team",
                "url": "https://falcon-sign.info/",
                "description": "Official Falcon specification, security proofs, and constant-time floating-point implementations.",
                "type": "spec"
            }
        ]
    },
    "code-based-mceliece": {
        ...
        "references": [
            ...
            {
                "title": "Classic McEliece: NIST Post-Quantum Standardization Submission",
                "source": "Classic McEliece Consortium",
                "url": "https://classic.mceliece.org/",
                "description": "Complete Round 4 submission specifications, parameters, and constant-time implementations.",
                "type": "spec"
            }
        ]
    },
    "hash-signatures": {
        ...
        "references": [
            ...
            },
            {
                ...
                "source": "IETF RFC",
                "url": "https://www.ietf.org/rfc/rfc8391.txt",
                "description": "Standard for stateful hash-based digital signatures used in secure firmware signing.",
                "type": "standard"
            }
        ]
    },
    "multivariate-isogeny": {
        ...
        "references": [
            ...
            {
                "title": "Ward Beullens: Breaking Rainbow Takes a Weekend on a Laptop (Crypto 2022)",
                "source": "IACR Crypto",
                "url": "https://eprint.iacr.org/2022/214",
                "description": "The MinRank algebraic attack that broke the NIST Round 3 finalist multivariate signature scheme Rainbow.",
                "type": "paper"
            }
        ]
    },
    "pqc-migration-hybrid": {
        ...
        "references": [
            ...
            },
            {
                ...
                "description": "Official US government advisory setting mandatory timelines for post-quantum migration through 2033.",
                "type": "spec"
            }
        ]
    },
    "lightweight-crypto": {
        ...
        "references": [
            ...
            {
                "title": "William Stallings: Cryptography and Network Security (8th Edition, Chapter 14)",
                "source": "Textbook Standard",
                "url": "https://en.wikipedia.org/wiki/Ascon_(cipher)",
                "description": "Chapter 14: Lightweight Cryptography, IoT security constraints, and PRESENT / ASCON architectures.",
                "type": "book"
            }
        ]
    },
    "saes-appendix-d": {
        ...
        "references": [
            ...
            {
                "title": "William Stallings: Cryptography and Network Security (8th Edition, Appendix D)",
                "source": "Textbook Standard",
                "url": "https://en.wikipedia.org/wiki/Advanced_Encryption_Standard",
                "description": "Appendix D: Comprehensive mathematical reference and tables for Simplified AES.",
                "type": "book"
            }
        ]
    }
...
# Read existing postquantum.ts
with open("d:/Projects/Crypto/Cypher/src/content/postquantum.ts", "r", encoding="utf-8") as f:
    content = f.read()

# For each lesson id, insert pitfalls, workedExample, and references before toolId or glossary or closing brace
for lesson_id, data in enrichments.items():
    # Regex to find this lesson
    pattern = rf'(id:\s*"{lesson_id}",[\s\S]*?)(toolId:|glossary:|\n  \}})'
    match = re.search(pattern, content)
    if not match:
        print(f"Warning: could not find {lesson_id}")
        continue
    
    insert_str = ""
    if "pitfalls" in data:
        pitfalls_json = json.dumps(data["pitfalls"], indent=6)
        insert_str += f"    pitfalls: {pitfalls_json},\n"
    if "workedExample" in data:
        we_json = json.dumps(data["workedExample"], indent=6)
        insert_str += f"    workedExample: {we_json},\n"
    if "references" in data:
        ref_json = json.dumps(data["references"], indent=6)
        insert_str += f"    references: {ref_json},\n"
    
    # Replace in content
    # We want to insert insert_str right before the next field
    prefix = match.group(1)
    next_field = match.group(2)
    new_block = f"{prefix}{insert_str}    {next_field}"
    content = content[:match.start()] + new_block + content[match.end():]

with open("d:/Projects/Crypto/Cypher/src/content/postquantum.ts", "w", encoding="utf-8") as f:
    f.write(content)

print("Successfully updated postquantum.ts")
