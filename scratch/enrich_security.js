...
const securityFilePath = 'd:/Projects/Crypto/Cypher/src/content/security.ts';
...
const baseLessons = [
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Spoofing", detail: "Attacker impersonates cardiac monitor. Mitigation: Mutual TLS (mTLS) with hardware-rooted X.509 certificates." },
        { label: "2. Tampering", detail: "Attacker alters telemetry in flight. Mitigation: Authenticated Encryption with Associated Data (AES-GCM)." },
        { label: "3. Repudiation", detail: "Clinic denies receiving emergency shock alert. Mitigation: Digitally signed telemetry logs in append-only tamper-evident hash chain." },
        { label: "4. Information Disclosure", detail: "Eavesdropper snoops patient ECG traces on hospital WiFi. Mitigation: End-to-end TLS 1.3 with Perfect Forward Secrecy." },
        { label: "5. Denial of Service", detail: "RF jamming or API flooder suppresses cardiac alarms. Mitigation: Dual-band backup transmitter, local flash logging, and rate limiting." },
        { label: "6. Elevation of Privilege", detail: "Malicious payload exploits telemetry parser. Mitigation: Sandboxed parser with unprivileged daemon user (least privilege)." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Inbound Request", detail: "GET /api/documents/1042 with Cookie: session=s_987654321 (Logged in as user_id=42)." },
        { label: "2. Vulnerable Query", detail: "SELECT * FROM documents WHERE id = 1042 (Doc 1042 belongs to user 99 — unauthorized access granted!)." },
        { label: "3. Enforced Authorization", detail: "SELECT * FROM documents WHERE id = 1042 AND (owner_id = 42 OR tenant_id IN (SELECT tenant_id FROM memberships WHERE user_id = 42 AND role = 'viewer'))." },
        { label: "4. Result Check", detail: "0 rows returned -> Handler issues HTTP 404 Not Found (or 403 Forbidden without leaking existence)." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Complex Pattern", detail: "Password 'P@ssw0rd2024!' has 13 chars. Naive theoretical entropy: 13 · log₂(95) = 85.4 bits." },
        { label: "2. Dictionary Reality", detail: "Hashcat rule 'Password' + '@' + '0' + 'year' + 'symbol' reduces effective search space to just 2²⁶ combinations." },
        { label: "3. Cracking Cost", detail: "On an 8x RTX 4090 rig hashing NTLM @ 100 GH/s: 'P@ssw0rd2024!' is cracked in 0.0006 seconds." },
        { label: "4. Diceware Passphrase", detail: "'correct horse battery staple' (4 words from 7,776-word list) carries 4 · log₂(7776) = 51.7 bits of true random entropy." },
        { label: "5. Memory-Hard Defense", detail: "Using Argon2id (m=64MB, t=3): cracking speed drops to 1,000 hashes/sec; cracking the 4-word passphrase takes over 100,000 years." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Hostile Input", detail: "User input string: admin@bank.com' OR '1'='1" },
        { label: "2. Vulnerable Concatenation", detail: "query = 'SELECT * FROM users WHERE email = '' + input + '' AND active = 1' (Syntax evaluated: returns all records and grants admin!)." },
        { label: "3. Parameterized Query", detail: "query = 'SELECT * FROM users WHERE email = ? AND active = 1'; preparedStatement.setString(1, input);" },
        { label: "4. Execution Boundary", detail: "SQL AST template is compiled first; parameter is transmitted as pure literal binary data over wire." },
        { label: "5. Engine Evaluation", detail: "Database searches literally for email matching 'admin@bank.com\\' OR \\'1\\'=\\'1'. 0 rows matched; access denied." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Vulnerable Code", detail: "for (let i=0; i<len; i++) { if (a[i] !== b[i]) return false; } (Returns false on first byte mismatch)." },
        { label: "2. Testing Byte 0", detail: "Attacker submits 256 requests with varying byte 0. When byte 0 matches, function checks byte 1, taking ~15 ns longer." },
        { label: "3. Statistical Analysis", detail: "Over 5,000 samples per byte, network jitter is averaged out. Byte 0 is identified with >99.9% statistical confidence." },
        { label: "4. Linear Complexity", detail: "Attacker repeats for bytes 1 through 31. Total complexity is only 32 · 256 = 8,192 requests rather than 2²⁵⁶ brute force." },
        { label: "5. Constant-Time Fix", detail: "let diff = 0; for (let i=0; i<len; i++) { diff |= (a[i] ^ b[i]); } return diff === 0; (No early exit; timing delta is 0 ns)." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Target Setup", detail: "Target block C₁, preceding block C₀. Decryption: P₁[15] = D_K(C₁)[15] ⊕ C₀[15]." },
        { label: "2. Craft Probe Block", detail: "Attacker creates C₀' with random bytes 0..14 and sweeps byte 15 through values 0x00 to 0xFF." },
        { label: "3. Oracle Query", detail: "Server decrypts D_K(C₁) ⊕ C₀' and checks PKCS#7 padding. For 255 values, padding is invalid (Error 500)." },
        { label: "4. Oracle Success", detail: "At C₀'[15] = 0x7E, decrypted last byte equals 0x01 (valid 1-byte padding, 200 OK or Bad Signature)." },
        { label: "5. Algebraic Recovery", detail: "D_K(C₁)[15] ⊕ 0x7E = 0x01 ⇒ D_K(C₁)[15] = 0x7E ⊕ 0x01 = 0x7F." },
        { label: "6. Plaintext Decoded", detail: "P₁[15] = D_K(C₁)[15] ⊕ C₀[15] = 0x7F ⊕ C₀[15]. Decoded in at most 256 requests!" },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Setup", detail: "Public group: prime p = 23, generator g = 5. Secret key x = 6. Public key y = 5⁶ mod 23 = 8." },
        { label: "2. Commitment", detail: "Prover picks random ephemeral nonce v = 4. Computes commitment t = gᵛ mod p = 5⁴ mod 23 = 4. Sends t to Verifier." },
        { label: "3. Challenge", detail: "Verifier sends random challenge c = 3 back to Prover." },
        { label: "4. Response", detail: "Prover calculates r = v - c·x mod (p - 1) = 4 - (3 · 6) = -14 ≡ 8 (mod 22). Sends r = 8 to Verifier." },
        { label: "5. Verification", detail: "Verifier checks: gʳ · yᶜ mod p == (5⁸ · 8³ mod 23) = (16 · 6 mod 23) = 96 mod 23 = 4 == t. Verification succeeds!" },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Generate Data Key", detail: "App requests KMS: GenerateDataKey(K_master, AES-256). KMS returns Plaintext DK and Encrypted DK (wrapped under master key)." },
        { label: "2. Encrypt Payload", detail: "App encrypts 500 MB record locally with Plaintext DK using AES-256-GCM." },
        { label: "3. Memory Cleansing", detail: "App overwrites Plaintext DK in memory with zeroes (secure zeroize)." },
        { label: "4. Database Storage", detail: "Database row stores: [Encrypted DK (48B) || IV (12B) || Ciphertext (500MB) || Auth Tag (16B)]." },
        { label: "5. Master Key Rotation", detail: "Yearly master key rotation: only the 48-byte Encrypted DK is re-wrapped; zero heavy database re-encryption needed." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. ClientHello", detail: "Client generates ephemeral private key x; sends ClientHello with supported ciphers (TLS_AES_256_GCM_SHA384) and key share gˣ." },
        { label: "2. ServerHello", detail: "Server generates ephemeral y; computes shared secret S = (gˣ)ʸ = gˣʸ. Sends ServerHello with key share gʸ." },
        { label: "3. Handshake Secret Derivation", detail: "Both derive Handshake Secret using HKDF-Extract and HKDF-Expand; all remaining handshake packets are encrypted." },
        { label: "4. Server Certificate & Verification", detail: "Server sends EncryptedCertificate and CertificateVerify (digital signature over transcript) and Finished HMAC." },
        { label: "5. Client Finished", detail: "Client validates X.509 chain against root CA trust store, sends ClientFinished, and switches to Application Traffic Keys." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Vulnerable Endpoint", detail: "POST /api/transfer { amount: 500, to: 'Alice' } with HMAC-SHA256 signature. Interceptor replays packet 100 times, draining $50,000." },
        { label: "2. Client Freshness Payload", detail: "Client attaches timestamp t = 1718000000 and 128-bit UUID nonce n = 'a7b3c8...'. Signature: HMAC(body || t || n)." },
        { label: "3. Timestamp Check", detail: "Server verifies: |now - t| < 60 seconds. Stale requests are rejected immediately." },
        { label: "4. Atomic Nonce Lock", detail: "Server executes Redis: SET nonce:a7b3c8... 1 EX 120 NX (Atomic insert with 120s TTL only if key does not already exist)." },
        { label: "5. Replay Rejection", detail: "If Redis returns nil (key was already present), server immediately rejects duplicate transaction with HTTP 409 Conflict." },
      ],
  ...
  },
  {
    ...
    workedExample: {
      ...
      steps: [
        { label: "1. Detection (MTTD: 3m)", detail: "SIEM detects abnormal SMB file-enumeration spikes and canary honeypot file encryption on file server SRV-01." },
        { label: "2. Automated Isolation", detail: "SOAR playbook disables SRV-01 switch port and revokes active Kerberos ticket-granting tickets within 90 seconds." },
        { label: "3. Volatile Evidence", detail: "Security engineer captures live RAM image over out-of-band IPMI to preserve memory keys and injected malware payloads." },
        { label: "4. Patient Zero Eradication", detail: "Incident team traces ingress to compromised VPN session lacking MFA; credentials purged and VPN gateway hardened." },
        { label: "5. Immutable Recovery", detail: "Clean system re-imaged from air-gapped immutable backups; integrity validated against pre-breach SHA-256 baseline hashes." },
      ],
...
export const securityLessons: Lesson[] = ${JSON.stringify(baseLessons, null, 2)};
`;

fs.writeFileSync(securityFilePath, fileContent, 'utf8');
console.log('Successfully written typed securityLessons to', securityFilePath);
